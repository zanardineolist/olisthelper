...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Escolher a lógica de filtragem com base no modo fornecido
    switch (mode) {
      case 'leaderboard':
        // Lógica do leaderboard: Filtrar registros com base no mês atual
        const currentDate = new Date();
        const currentMonth = currentDate.getMonth();
        const currentYear = currentDate.getFullYear();

        const filteredLeaderboardRows = rows.filter((row, index) => {
        ...
        console.log(`Total de registros filtrados para leaderboard: ${filteredLeaderboardRows.length}`);

        return res.status(200).json({ rows: filteredLeaderboardRows });

      case 'profile':
        // Lógica de perfil: Contar registros do mês atual e do mês anterior
        const currentProfileDate = new Date();
        const currentProfileMonth = currentProfileDate.getMonth() + 1;
        const currentProfileYear = currentProfileDate.getFullYear();
        const lastMonth = currentProfileMonth === 1 ? 12 : currentProfileMonth - 1;
        const lastMonthYear = currentProfileMonth === 1 ? currentProfileYear - 1 : currentProfileYear;
        ...
        rows.forEach((row, index) => {
          ...
          if (year === currentProfileYear && month === currentProfileMonth) {
            currentMonthCount++;
      ...
      default:
        // Lógica padrão: Filtrar registros gerais (últimos 30 dias ou baseado no filtro fornecido)
        const brtDate = new Date().toLocaleString("en-US", { timeZone: "America/Sao_Paulo" });
        const currentGeneralDate = new Date(brtDate);

        const filteredGeneralRows = rows.filter((row, index) => {
          if (index === 0) return false; // Pular cabeçalho
          ...
          const diffTime = currentGeneralDate - date;
          const diffDays = diffTime / (1000 * 60 * 60 * 24);

          return diffDays <= (filter ? parseInt(filter, 10) : 30);
        });

        if (!filteredGeneralRows || filteredGeneralRows.length === 0) {
          console.log('Nenhum registro encontrado após o filtro aplicado.');
          return res.status(200).json({ count: 0, dates: [], counts: [], rows: [] });
        }

        console.log(`Total de registros após o filtro aplicado: ${filteredGeneralRows.length}`);

        const count = filteredGeneralRows.length;
        const dates = filteredGeneralRows.map((row) => row[0]);
